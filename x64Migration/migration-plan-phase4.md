...
9. **Universe-screen ship lighting variability (deferred)** — *(surfaced during §4.6 #10 work, 2026-05-07)* Some ship hulls render as black silhouettes on the Universe screen while others light correctly. Variability across hulls argues for a per-mesh lighting-binder gap (`LightingEffectBinder` not pushing the per-system point lights to every SO equally), not a global pipeline issue. Diagnostic toolkit from §4.5.A (forced-color PS, parameter introspection) ports over directly. Bounded scope; not blocking gameplay.
