...
namespace UnitTests.Ships
{
    ...
    public class TestWeaponArcs : StarDriveTest
    {
        ...
        public void FiringOutsideOfArc()
        {
            ...
            Assert.AreEqual(0, Universe.GetProjectilesForShip(ship).Count, "No projectiles should launch");
        }
        ...
        public void FiringPointDefenseAtMissiles()
        {
            ...
            Array<Projectile> projectiles = Universe.GetProjectilesForShip(enemy);
            Assert.IsTrue(weapon.UpdateAndFireAtTarget(enemy, projectiles, NoShips), "Fire PD at a projectile must succeed");
            Assert.AreEqual(1, Universe.GetProjectilesForShip(us).Count, "Invalid projectile count");
