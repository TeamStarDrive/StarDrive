  ...
  internal class Class8
  {
    public static string smethod_0(byte[] byte_0)
    {
      ...
      if (num1 > 0)
      {
        ...
        byte_0.CopyTo((Array) numArray1, 0);
      }
    ...
    public static byte[] smethod_1(string string_0)
    {
      ...
      for (int index = 0; index < bytes1.Length; ++index)
        bytes1[index] = (int) bytes1[index] >= 65 ? (byte) ((uint) bytes1[index] - 55U) : (byte) ((uint) bytes1[index] - 48U);
      int index1 = 0;
      while (index1 < bytes1.Length)
      {
        byte[] bytes2 = BitConverter.GetBytes((int) bytes1[index1] & 31 | ((int) bytes1[index1 + 1] & 31) << 5 | ((int) bytes1[index1 + 2] & 31) << 10 | ((int) bytes1[index1 + 3] & 31) << 15 | ((int) bytes1[index1 + 4] & 31) << 20 | ((int) bytes1[index1 + 5] & 31) << 25 | ((int) bytes1[index1 + 6] & 3) << 30);
        byte[] numArray2 = numArray1;
        int index2 = num1;
        int num2 = 1;
        int num3 = index2 + num2;
        int num4 = (int) bytes2[0];
