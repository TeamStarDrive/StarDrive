...
namespace Ship_Game.AI
{
    public class MilitaryTask : IDisposable
    {
        ...
        {
            var ships =area.GetOffensiveForcePool().Union(Empire.GetForcePool());
        ...
        public void EndTask()
        {
            ...
            if (WhichFleet != -1)
            {
                ...
                {
                    TaskForce.Clear();
                    if (Fleet.IsCoreFleet || Empire.isPlayer)
                        return;
            ...
            Array<Troop> potentialTroops,float EnemyTroopStrength, AO closestAO,  Array<Ship> potentialBombers = null, string fleetName = "Invasion Fleet")
        {
            ...
            foreach (Ship ship in elTaskForce)
            {
                ...
                Empire.GetGSAI().RemoveShipFromForce(ship,closestAO);
            }
        ...
        {            
            var aos = Empire.GetGSAI().AreasOfOperations;
            if(aos == null)
            {
