...
namespace Ship_Game
{
	public class FloatSlider
	{
        ...
        public void Draw10(Ship_Game.ScreenManager ScreenManager)
        {
            Microsoft.Xna.Framework.Graphics.SpriteBatch SpriteBatch = ScreenManager.SpriteBatch;
            Vector2 Cursor = new Vector2((float)(this.ContainerRect.X + 10), (float)this.ContainerRect.Y);
            SpriteBatch.DrawString(Fonts.Arial12Bold, this.Text, Cursor, new Color(255, 239, 208));
            ...
            for (int i = 0; i < 11; i++)
            {
                tickCursor = new Vector2((float)(this.rect.X + this.rect.Width / 1000 * i), (float)(this.rect.Y + this.rect.Height + 2));
                if (this.Hover)
                {
                    SpriteBatch.Draw(ResourceManager.TextureDict["NewUI/slider_minute_hover"], tickCursor, Color.White);
                }
        ...
        public float HandleInput10(InputState input)
        {
            if (!HelperFunctions.CheckIntersection(this.rect, input.CursorPosition))
            {
                this.Hover = false;
            ...
            if (this.dragging)
            {
                ...
                this.amount = 1f - (float)((float)this.rect.X + (float)this.rect.Width - (float)this.cursor.X) / ( (float)this.rect.Width*10);
            }
            return this.amount;
        }
        ...
        public void SetAmount10(float amt)
        {
            this.amount = amt;
            this.cursor = new Rectangle(this.rect.X + (int)((float)this.rect.Width / 10f ), this.rect.Y + this.rect.Height / 2 - ResourceManager.TextureDict["NewUI/slider_crosshair"].Height / 2, ResourceManager.TextureDict["NewUI/slider_crosshair"].Width, ResourceManager.TextureDict["NewUI/slider_crosshair"].Height);
        }
	}
}