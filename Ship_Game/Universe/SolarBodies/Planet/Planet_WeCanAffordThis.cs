    ...
    public partial class Planet
    {
        ...
        bool CanAffordAgricultural(in Afford a, Building b)
        {
            if (b.AllowShipBuilding && Prod.NetMaxPotential > 20)
                return true;
            if (Fertility(Owner) > 0 && b.MaxFertilityOnBuild < 0 && NonCybernetic)
                return true;
            if (a.HighPri)
            {
                if (b.PlusFlatFoodAmount > 0
                    ...
                    || ((b.MaxPopIncrease > 0
                    || b.PlusFlatPopulation > 0 || b.PlusTerraformPoints > 0) && Population > MaxPopulation(Owner) * 0.5f)
            ...
            {
                if (b.IsBiospheres || (b.PlusTerraformPoints > 0 && Fertility(Owner) < 3)
                                   || b.MaxPopIncrease > 0
                                   || b.PlusFlatPopulation > 0
                                   || IsCoreWorld
                                   || b.PlusFlatResearchAmount > 0
                                   || (b.PlusResearchPerColonist > 0 && MaxPopulation(Owner) > 999)
                                   || a.NeedDefense)
                    return true;
            }
        ...
        bool CanAffordCore(in Afford a, Building b)
        {
            ...
            {
                if (b.StorageAdded > 0
                    || (NonCybernetic && (b.PlusTerraformPoints > 0 && Fertility(Owner) < 1) && MaxPopulation(Owner) > 2000)
                    || ((b.MaxPopIncrease > 0 || b.PlusFlatPopulation > 0)
                        && Population < MaxPopulation(Owner)*0.5f && Money.NetRevenue*0.5f > b.Maintenance)
        ...
        bool CanAffordIndustrial(in Afford a, Building b)
        {
            ...
            if (a.HighPri)
            {
                if (b.PlusFlatProductionAmount > 0
                    ...
                    || b.PlusProdPerColonist > 0
                    || b.PlusFlatProductionAmount > 0
                    || (NonCybernetic && Fertility(Owner) < 1f && b.PlusFlatFoodAmount > 0)
                    || b.StorageAdded > 0
                    || (a.NeedDefense && IsCoreWorld))
                    return true;
            }
            ...
            if (a.MedPri && IsVibrant && a.MakingMoney)
            {
                if ((b.PlusResearchPerColonist * PopulationBillion) > b.Maintenance
                    || ((b.MaxPopIncrease > 0 || b.PlusFlatPopulation > 0) && Population.AlmostEqual(MaxPopulation(Owner)))
                    || (NonCybernetic && b.PlusTerraformPoints > 0 && Fertility(Owner) < 1 && Population.AlmostEqual(MaxPopulation(Owner)) &&
                        MaxPopulation(Owner) > 2000)
        ...
        bool CanAffordMilitary(in Afford a, Building b)
        {
            ...
            if (a.HighPri)
            {
                if (b.isWeapon
                    || b.IsSensor
                    || b.Defense > 0
                    || (Fertility(Owner) < 1f && b.PlusFlatFoodAmount > 0)
        ...
        bool CanAffordForResearch(in Afford a, Building b)
        {
            ...
            if (a.MedPri && IsCoreWorld && a.MakingMoney)
            {
                if (((b.MaxPopIncrease > 0 || b.PlusFlatPopulation > 0) && Population > MaxPopulation(Owner) * .5f)
                    || NonCybernetic && ((b.PlusTerraformPoints > 0 && Fertility(Owner) < 1 && Population > MaxPopulation(Owner) * 0.5f && MaxPopulation(Owner) > 2000)
                    || (b.PlusFlatFoodAmount > 0 && Food.NetIncome < 0)))
                    return true;
            }
            if (a.LowPri && IsMegaWorld && a.MakingMoney)
