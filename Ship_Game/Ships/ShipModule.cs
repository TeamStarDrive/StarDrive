...
namespace Ship_Game.Ships
{
    public sealed class ShipModule : GameplayObject
    {
        ...
        void CauseSiphonDamage(Beam beam)
        {
            ...
            ShieldPower = ShieldPower.Clamped(0, ShieldPowerMax);
        ...
        public void VisualizeRepair()
        {
            if (Parent.IsVisibleToPlayer)
            {
                ...
                Vector3 parentVelocity = Parent.Velocity.ToVec3(-5f);
                var p = Parent.Universe.Particles;
                for (int i = 0; i < 5; i++)
                    p.BlueSparks.AddParticle(repairEffectOrigin, parentVelocity);
            }
        }
