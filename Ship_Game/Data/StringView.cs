...
namespace Ship_Game.Data
{
    public struct StringView : IEquatable<StringView>
    {
        ...
        /**
         * This is an iterative version of a String.Split
         * Each call to Next('=') will result in next sub-token being parsed.
         * Example:
         * view = StringView("key=value;key2=value2");
         * view.Next('=') --> "key"
         * view.Next(';') --> "value"
         * view.Next('=') --> "key2"
         * view.next(';') --> "value2"
         */
        public StringView Next(char delimiter)
        {
            var next = new StringView(Chars, Start, 0);

            int end = Start + Length;
            for (int i = Start; i < end; ++i)
            {
                // found a match, update THIS instance
                ...
                // no match yet, expand the span of 'next'
                next.Length += 1;
            }

            // we didn't find the delimiter, so we should assign ALL to `next` and reset THIS
            Start = end;
            Length = 0;
            return next;
        }
