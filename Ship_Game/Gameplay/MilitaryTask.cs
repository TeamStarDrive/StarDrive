...
namespace Ship_Game.Gameplay
{
	public sealed class MilitaryTask: IDisposable
	{
            ...
            switch (this.type)
            {
                ...
                case MilitaryTask.TaskType.AssaultPlanet:
                    switch (this.Step)
                    {
                        ...
                        case 1:
                            Fleet fleet;
                            if(this.empire.GetFleetsDict().TryGetValue(this.WhichFleet ,out fleet))
                            {
        ...
        private float GetEnemyStrAtTarget()
        {
            ...
            float MinimumEscortStrength = standardMinimum;
            //300000;
		...
		private void RequisitionAssaultForces()
        {
            ...
            if (sorted.Count<Ship_Game.Gameplay.AO>() == 0)     return;
            ...
            { //add a danger timer qualifier.
                if (!planet1.HasShipyard)  continue;
            ...
            if (planets.Count<Planet>() == 0)   return;
            ...
            {
                if (PotentialTroops.Count > 30)     break;
                ...
                {
                    if (t.GetOwner() != this.empire)    continue;
                    t.SetPlanet(planet2);
                    extra--;
                    if(extra < 0)                    
                    PotentialTroops.Add(t);
            ...
            if (ourAvailableStrength >= EnemyTroopStrength &&  landingSpots >8 && troopCount >=10   && tfstrength >= this.MinimumTaskForceStrength)
            {
                ...
                for (int i = 1; i < 10 && !this.empire.GetGSAI().UsedFleets.Contains(i); i++)       //Can only have 10 fleets, and dont want to overwrite an existing one
                {
                    float ForceStrength = 0f;

                    foreach (Ship ship in PotentialAssaultShips)
                    {
                        if (ForceStrength > EnemyTroopStrength * 1.25f || landingSpots <= -5) break;
            ...
            else if (landingSpots < 10 && tfstrength >= this.MinimumTaskForceStrength && PotentialBombers.Count >0) //&& ourAvailableStrength >= EnemyTroopStrength
            {
                ...
                for (int i = 1; i < 10 && !this.empire.GetGSAI().UsedFleets.Contains(i); i++)
                {
                    float ForceStrength = 0f;
        ...
        private void RequisitionClaimForce()
        {
            ...
            if (shipCount < 3 && tfstrength < strengthNeeded)   return;
