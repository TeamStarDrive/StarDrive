	...
	public sealed class ArtificialIntelligence : IDisposable
	{
        ...
        */
        ...
        {            
            ...
            float DistCenter = Vector2.Distance(OrbitTarget.Position, this.Owner.Center);
            if (DistCenter > 15000.0)
            {
            ...
            */
            ...
            if ((double)this.findNewPosTimer <= 0.0)
            {
                if (distanceToOrbitSpot <= radius || this.Owner.speed == 0)
                {                    
                    this.OrbitalAngle += MathHelper.ToDegrees((float)Math.Asin(this.Owner.yBankAmount*10));
                    if ((double)this.OrbitalAngle >= 360.0)
                        this.OrbitalAngle -= 360f;
                }
                this.findNewPosTimer =  elapsedTime * 10;
                this.OrbitPos = this.GeneratePointOnCircle(this.OrbitalAngle, OrbitTarget.Position, radius);
            }
            else
                this.findNewPosTimer -= elapsedTime;

            if (distanceToOrbitSpot < 7500.0)
            {
                this.Owner.HyperspaceReturn();
                if (this.State != AIState.Bombard)
                    this.HasPriorityOrder = false;
            }
            if (distanceToOrbitSpot <= this.Owner.Radius + 500f)
            {
                this.ThrustTowardsPosition(this.OrbitPos, elapsedTime, this.Owner.speed > 500 ? 500 : this.Owner.speed);
            }
            else
                {
                    this.ThrustTowardsPosition(this.OrbitPos, elapsedTime, this.Owner.speed);
                            ...
                            if (item != null)
                            {
                                ...
                                if (this.Owner.AreaOfOperation.Count > 0)
                                {
                                    foreach (Rectangle areaOfOperation in this.Owner.AreaOfOperation)
                                    {
                                        ...
                                        for (int j = 0; j < this.Owner.loyalty.GetShips().Count; j++)
                                        {
                                            ...
                                            if (ship != null && (ship.Role == "freighter" || ship.shipData.ShipCategory == ShipData.Category.Civilian) && ship != this.Owner)
                                            {
                                ...
                                else if (item.fs == Planet.GoodState.IMPORT && item.FoodHere < item.MAX_STORAGE * 0.6f)
                                {
                                    ...
                                    for (int k = 0; k < this.Owner.loyalty.GetShips().Count; k++)
                                    {
                                        ...
                                        if (item1 != null && (item1.Role == "freighter" || item1.shipData.ShipCategory == ShipData.Category.Civilian) && item1 != this.Owner)
                                        {
                            ...
                            if (planet != null && planet.ParentSystem.combatTimer < 1)
                            {
                                if (this.Owner.AreaOfOperation.Count > 0)
                                {
                                    foreach (Rectangle rectangle in this.Owner.AreaOfOperation)
                                    {
                                        ...
                                        for (int m = 0; m < this.Owner.loyalty.GetShips().Count; m++)
                                        {
                                            ...
                                            if (ship1 != null)
                                            {
                                                if (ship1.Role == "freighter" || ship1.shipData.ShipCategory == ShipData.Category.Civilian)
                                                {
                                                    if (ship1 == this.Owner)
                                                    {
                                                        goto Label1;
                                ...
                                else if (planet.ps == Planet.GoodState.IMPORT && planet.ProductionHere < planet.MAX_STORAGE * 0.75f)
                                {
                                    ...
                                    for (int n = 0; n < this.Owner.loyalty.GetShips().Count; n++)
                                    {
                                        ...
                                        if (item2 != null)
                                        {
                                            if (item2.Role == "freighter" || item2.shipData.ShipCategory == ShipData.Category.Civilian)
                                            {
                                                if (item2 == this.Owner)
                                                {
                                                    goto Label0;
                        ...
                        if (planet1 != null && planet1.ParentSystem.combatTimer < 1)
                        {
                            if (this.Owner.AreaOfOperation.Count > 0)
                            {
                                foreach (Rectangle areaOfOperation1 in this.Owner.AreaOfOperation)
                                {
                                    ...
                                    for (int p = 0; p < this.Owner.loyalty.GetShips().Count; p++)
                                    {
                                        ...
                                        if (ship2 != null && (ship2.Role == "freighter" || ship2.shipData.ShipCategory == ShipData.Category.Civilian) && ship2 != this.Owner)
                                        {
                            ...
                            else if (planet1.fs == Planet.GoodState.IMPORT && planet1.FoodHere < planet1.MAX_STORAGE * 0.6f)
                            {
                                ...
                                for (int q = 0; q < this.Owner.loyalty.GetShips().Count; q++)
                                {
                                    ...
                                    if (item3 != null && (item3.Role == "freighter" || item3.shipData.ShipCategory == ShipData.Category.Civilian) && item3 != this.Owner)
                                    {
                        ...
                        if (planet3 != null && planet3.ParentSystem.combatTimer < 1)
                        {
                            if (this.Owner.AreaOfOperation.Count > 0)
                            {
                                foreach (Rectangle areaOfOperation2 in this.Owner.AreaOfOperation)
                                {
                                    ...
                                    for (int t = 0; t < this.Owner.loyalty.GetShips().Count; t++)
                                    {
                                        ...
                                        if (ship3 != null)
                                        {
                                            if (ship3.Role == "freighter" || ship3.shipData.ShipCategory == ShipData.Category.Civilian)
                                            {
                                                if (ship3 == this.Owner)
                                                {
                                                    goto Label3;
                            ...
                            else if (planet3.ps == Planet.GoodState.IMPORT && planet3.ProductionHere < planet3.MAX_STORAGE * 0.75f)
                            {
                                ...
                                for (int u = 0; u < this.Owner.loyalty.GetShips().Count; u++)
                                {
                                    ...
                                    if (item4 != null)
                                    {
                                        if (item4.Role == "freighter" || item4.shipData.ShipCategory == ShipData.Category.Civilian)
                                        {
                                            if (item4 == this.Owner)
                                            {
                                                goto Label2;
...
//#endif
            ...
            if ((((this.Owner.Role == "freighter" || this.Owner.shipData.ShipCategory == ShipData.Category.Civilian) && this.Owner.CargoSpace_Max > 0) || this.Owner.Role == "scout" || this.Owner.Role == "construction" || this.Owner.Role == "troop" || this.IgnoreCombat || this.State == AIState.Resupply || this.State == AIState.ReturnToHangar || this.State == AIState.Colonize) || this.Owner.VanityName == "Resupply Shuttle")
            {
                return;
            }
		...
		private void SetCombatStatusorig(float elapsedTime)
		{
            ...
            if ((this.Owner.Role == "freighter" || this.Owner.shipData.ShipCategory == ShipData.Category.Civilian || this.Owner.Role == "scout" || this.Owner.Role == "construction" || this.Owner.Role == "troop" || this.IgnoreCombat || this.State == AIState.Resupply || this.State == AIState.ReturnToHangar) && !this.Owner.IsSupplyShip)
			{
				return;
			}
			if (this.Owner.fleet != null && this.State == AIState.FormationWarp)
