            ...
            switch (this.Step)
            {
                case 0:
                    ...
                    for (int index = 0; index < this.empire.GetShips().Count; ++index)
                    {
                        ...
                        if (ship != null && !ship.isColonyShip && ((ship.shipData.Role == ShipData.RoleName.freighter || ship.shipData.ShipCategory == ShipData.Category.Civilian) && !ship.isPlayerShip()) && (ship.GetAI() != null && ship.GetAI().State != AIState.PassengerTransport && ship.GetAI().State != AIState.SystemTrader))
                        {
                            this.freighter = ship;
                            flag1 = true;
                        }
                ...
                case 2:
                    ...
                    foreach (Ship ship in (List<Ship>)this.empire.GetShips())
                    {
                        if (!ship.isColonyShip && (ship.shipData.Role == ShipData.RoleName.freighter || ship.shipData.ShipCategory == ShipData.Category.Civilian) && (!ship.isPlayerShip() && ship.GetAI() != null) && (ship.GetAI().State != AIState.PassengerTransport && ship.GetAI().State != AIState.SystemTrader && (!ship.GetAI().HasPriorityOrder && ship.GetAI().State != AIState.Refit)) && ship.GetAI().State != AIState.Scrap)
                        {
                            this.freighter = ship;
                            flag2 = true;
                        }
        ...
        private void DoIncreasePassTranGoal()
        {
            switch (this.Step)
            {
                case 0:
                    ...
                    foreach (Ship ship in (List<Ship>)this.empire.GetShips())
                    {
                        if (!ship.isColonyShip && (ship.shipData.Role == ShipData.RoleName.freighter || ship.shipData.ShipCategory == ShipData.Category.Civilian) && (!ship.isPlayerShip() && ship.GetAI() != null) && (ship.GetAI().State != AIState.PassengerTransport && ship.GetAI().State != AIState.SystemTrader))
                        {
                            this.passTran = ship;
                            flag1 = true;
                        }
